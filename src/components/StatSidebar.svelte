...
from-indigo-600 from-10 to-indigo-800 mx-auto my-2 md:my-4 md:mx-4 md:min-h-screen shadow-lg px-4 py-10 flex flex-row md:flex-col
items-center rounded-2xl gap-4 md:gap-0"
>
	{#if $session.loading}
	...
	{:else if $session.user}
		<div class="flex flex-col gap-2 md:gap-4 items-center mr-5 md:mr-0">
			<div class="aspect-square w-24 h-24 md:w-36 md:h-36">
				<img
					class="border-4 md:border-8 border-white/30 shadow-md w-full h-full object-cover overflow-hidden rounded-full"
					width={0}
					height={0}
