 ...
 */
export async function runAndRetryOnThrow<T>(f: () => Promise<T> | T, retryFn: RetryFunction, log = true): Promise<T> {
    let retryCount = 0;
...
export abstract class Retry {
     ...
     */
    static exponential({ start = 1, factorMs = 1000, attends }: { start?: number, factorMs?: number, attends?: number } = {}): RetryFunction {
     ...
     */
    static random({ min = 0, max, attends }: { min?: number, max: number, attends?: number }): RetryFunction {
