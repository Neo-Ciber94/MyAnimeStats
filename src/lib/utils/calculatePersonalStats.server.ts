...
function calculateStrength(animeList: AnimeObjectWithStatus[]) {
	...
	const matches = animeList.filter(({ node }) => {
		return (node.genres || []).some((x) =>
			STRENGTH_GENRES.map((genre) => genre.ID as number).includes(x.id)
		);
	});
...
function calculateCharisma(animeList: AnimeObjectWithStatus[]) {
	...
	const matches = animeList.filter(({ node }) => {
		return (node.genres || []).some((x) =>
			CHARISMA_GENRES.map((genre) => genre.ID as number).includes(x.id)
		);
	});
...
function calculateIntelligence(animeList: AnimeObjectWithStatus[]) {
	...
	const matches = animeList.filter(({ node }) => {
		return (node.genres || []).some((x) =>
			INTELLIGENCE_GENRES.map((genre) => genre.ID as number).includes(x.id)
		);
	});
...
function calculateVitality(animeList: AnimeObjectWithStatus[]) {
	...
	const matches = animeList.filter(({ node }) => {
		return (node.genres || []).some((x) =>
			VITALITY_GENRES.map((genre) => genre.ID as number).includes(x.id)
		);
	});
