	...
	export async function getUserDetails(username: string): Promise<User | null> {
		...
		try {
			...
			const meanScore = Number(
				htmlDocument
					.querySelector('.user-statistics-stats .stat-score .score-label')
					?.textContent?.trim()
			);

			// TODO: This should not be hardcoded
			const joinedAt = htmlDocument
				.querySelectorAll('ul.user-status li span.fl-r')?.[4]
				?.textContent?.trim();

			const getStat = (status: WatchStatus) => {
				const text = htmlDocument
					.querySelector(`.stats-status a.anime.${status} + span`)
