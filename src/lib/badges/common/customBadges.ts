...
const customBadges = [
		...
		canHaveBadge(animeList) {
			return (
				Enumerable.from(animeList)
					.where((x) => hadWatchedAnime(x))
					.where((x) =>
						(x.node.genres || []).some((genre) => genre.id === ANIME_GENRES.BoysLove.ID)
					)
					.count() >= 20
			);
		}
		...
		canHaveBadge(animeList) {
			return (
				Enumerable.from(animeList)
					.where((x) => hadWatchedAnime(x))
					.where((x) =>
						(x.node.genres || []).some((genre) => genre.id === ANIME_GENRES.GirlsLove.ID)
					)
					.count() >= 20
			);
		}
		...
		canHaveBadge(animeList) {
			...
			return jojoBizarreAdventureIds.every((animeId) => {
				return animeList.some(
					({ node, list_status }) => node.id === animeId && list_status.status === 'completed'
				);
			});
		}
	},
	...
	},
	{
		...
		name: (user) =>
			/*html*/ `<span>Watashi wa <b class="text-violet-500">${user.name}</b> desu</span>`,
